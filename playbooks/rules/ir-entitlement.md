...
### IR SLA 判断方式
- **唯一标准**：D365 Performance Indicators 的 IR SLA 状态
- **API 可用**：通过 `msdfm_caseperfattributes` 实体查询（`incident` 通过 `msdfm_caseperfattributesid` 关联）
  - `msdfm_initialresponse` / `msdfm_firstdayresolution` / `msdfm_firstweekresolution`
  - 值：`847050000` = Pending(In Progress), `847050001` = Met(Succeeded), `847050002` = Missed(Expired)
  - 额外字段：`msdfm_irslaremainingtime`, `msdfm_initialresponseexpirationtime`, `msdfm_initialresponsecompletedon`
- **~~OData API 不可用~~**（已过时）：`incident` 实体自身的 `msdfm_initialresponse` / `firstresponseslastatus` 字段确实全返回 null / "In Progress"，不能用。但 `msdfm_caseperfattributes` 实体的同名字段是可用且准确的。
- **检查方式**：
  - **单个 Case**：`check-ir-status.ps1`（API 优先，UI scraping 降级）
  - **批量全部 Case**：`check-ir-status-batch.ps1`（单次 API ~2s，推荐巡检使用）
...
| 方式 | 脚本 | 速度 | 场景 |
|------|------|------|------|
| **API**（推荐） | `check-ir-status.ps1`（默认 `-UseApi`）或 `check-ir-status-batch.ps1` | ~2s（批量全部 case） | 巡检、常规检查 |
| **UI scraping**（降级） | `check-ir-status.ps1 -UseApi:$false` | ~15-20s/case | API 失败时自动降级 |

**API 查询的实体**：`msdfm_caseperfattributes`（通过 `incident.msdfm_caseperfattributesid` 关联）

**API vs UI 差异**：
- API 不提供 "Nearing" 状态（仅 UI timer 有），API 下 Nearing 会显示为 "In Progress"
- API 提供 `expirationTime` 和 `remaining` 字段，上层可据此自行判断是否接近超时
- UI 提供实时倒计时 remaining 字符串（如 "2h 30m"）
...
| `Nearing` | `Nearing` | 接近超时（仅 UI 模式） |
| `Expired` | `Expired` | SLA 超时 |
| `unknown` | `unknown` | 数据缺失或检查失败 |

### meta 额外字段（API 模式）
- `source`: `"api"` 或 `"ui"`，标记数据来源
- `expirationTime`: IR SLA 到期时间（ISO 8601）
- `completedOn`: IR SLA 完成时间（如已 Met）

### 缓存跳过
- IR SLA 为 `Succeeded` 是终态，不会再变化
- `fetch-all-data.ps1` 检测到 meta 中 `irSla.status === "Succeeded"` 时跳过检查，节省 ~20s

### 批量检查（巡检推荐）
```powershell
# 单次 API 调用获取所有活跃 case 的 KPI，写入各 case 的 meta
pwsh -File check-ir-status-batch.ps1 -SaveMeta -MetaDir "$casesRoot\active"
```
- 巡检（patrol）模式下，先运行 batch 脚本预填所有 meta，后续 data-refresh 可跳过 IR check

### 规则
- IR/FDR/FWR 状态值原样写入，**不做 kebab-case 转换**（详见 `case-data-schema.md`）
- data-refresh 写入 `irSla`/`fdr`/`fwr`，compliance-check 不覆盖这些字段
- 如果脚本执行失败（API + UI 都失败），沿用已有值
