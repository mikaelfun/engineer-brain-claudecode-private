...
Help developers transform software ideas into comprehensive PRD.md files through structured questioning.
...
- Use AskUserQuestion tool to ask about a11y (only include a11y tasks if the user requests it)
- Use AskUserQuestion tool to ask about performance considerations (only include if requested)
...
Create PRD containing all identified requirements and use an ID to track each requirement.
...
Save to user-specified location (default: project root `PRD.md`).
...
"Your PRD is complete and saved. Would you like me to proceed to the next step and generate implementation tasks? See [JSON.md](JSON.md)."
