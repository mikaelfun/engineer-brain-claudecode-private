...
- **`config.json` 不存在或无 `dataRoot`（首次使用）** → 执行 onboarding：
  1. 用 `AskUserQuestion` 询问导出目录路径（提示示例：`~/projects/data/shared/onenote/export`）
...
- `pages` 为数组 → 对每个条目调用 `-SectionPath "{section}" -PageName '^{page}$'`
  - **不要预转义** `[]` 等特殊字符——PS1 脚本内部已用 `[regex]::Escape()` 处理
  - 直接传原始 page name，用 `^` `$` 包裹做精确匹配即可
...
powershell -NoProfile -STA -File .claude/skills/onenote/scripts/export-onenote.ps1 -NotebookName "{notebookName}" -OutputBase "{outputDir}"

# sections = ["Identity", "Storage"] → 逐 section
powershell -NoProfile -STA -File .claude/skills/onenote/scripts/export-onenote.ps1 -NotebookName "{notebookName}" -SectionPath "Identity" -OutputBase "{outputDir}"
powershell -NoProfile -STA -File .claude/skills/onenote/scripts/export-onenote.ps1 -NotebookName "{notebookName}" -SectionPath "Storage" -OutputBase "{outputDir}"
```

**处理 `pages`**（section + page 级别的精确范围）：
- `pages` 为数组 → 对每个 `{section, page}` 条目，传 `-SectionPath "{section}" -PageName '^{page}$'`
- **不要预转义** `[]` 等特殊字符——PS1 脚本内部已用 `[regex]::Escape()` 处理
- `-PageName` 使用 `^...$` 包裹做精确匹配，避免匹配到同名子串
- 脚本内置逻辑会自动包含匹配 page 的所有 sub-page

```bash
# pages = [{"section": "Manual", "page": "Topic List"}]
powershell -NoProfile -STA -File .claude/skills/onenote/scripts/export-onenote.ps1 -NotebookName "{notebookName}" -SectionPath "Manual" -PageName '^Topic List$' -OutputBase "{outputDir}"

# pages = [{"section": "[Team]New Hire Manual", "page": "[Team]Soft Skill trainings"}]
# 不需要转义 [] — PS1 脚本内部 [regex]::Escape() 会处理
powershell -NoProfile -STA -File .claude/skills/onenote/scripts/export-onenote.ps1 -NotebookName "{notebookName}" -SectionPath "[Team]New Hire Manual" -PageName '^[Team]Soft Skill trainings$' -OutputBase "{outputDir}"
```
...
5. **超大 section（1000+ 页）使用 `run_in_background: true`** — Bash tool 最大 timeout 为 10 分钟，超大 section 可能需要 30 分钟以上。使用后台运行避免超时，完成后通过通知获取结果。如果用前台模式被 timeout 中断，脚本增量模式下已导出的页面不会丢失，重跑即可续传

正确的调用方式：

```bash
# 链接模式
powershell -NoProfile -STA -File .claude/skills/onenote/scripts/export-onenote.ps1 -Link "{link}" -OutputBase "{outputDir}"

# 具名参数模式（增量）
powershell -NoProfile -STA -File .claude/skills/onenote/scripts/export-onenote.ps1 -NotebookName "{notebookName}" -OutputBase "{outputDir}"

# 带 section/page 过滤
powershell -NoProfile -STA -File .claude/skills/onenote/scripts/export-onenote.ps1 -NotebookName "{notebookName}" -SectionPath "{sectionPath}" -PageName "{pageName}" -OutputBase "{outputDir}"

# 强制全量重导出
powershell -NoProfile -STA -File .claude/skills/onenote/scripts/export-onenote.ps1 -NotebookName "{notebookName}" -OutputBase "{outputDir}" -Force
```
...
```bash
# 检查开关
# 注意：避免在 node -e "..." 中使用 !== 和 ?.（Bash 转义 ! 后 Node v24 TS parser 会报错）
AUTO_RAG=$(node -e "try{const c=JSON.parse(require('fs').readFileSync('config.json','utf-8'));console.log(c.onenote&&c.onenote.autoRagSync===false?'false':'true')}catch(e){console.log('true')}")
```
...
`outputDir` 可能包含 symlink，而 `local-rag` 内部用 `realpathSync` 解析文件路径会跟随 symlink。如果 `--base-dir` 仍用原始路径，`startsWith` 检查会失败。因此 `--base-dir` 和文件路径都必须用 resolved 后的真实路径：

```bash
# 解析 outputDir 的真实路径（跟随 symlink），取 parent 作为 base-dir
# 注意：realpathSync 在 Windows 返回反斜杠路径，必须转为正斜杠，
# 否则 Bash 双引号展开会把 \U \f 等当转义符吃掉
REAL_ONENOTE=$(node -e "console.log(require('fs').realpathSync('$OUTPUT_DIR'))" | sed 's|\\\\|/|g')
...
整个 skill 自包含在 `.claude/skills/onenote/` 目录下：

```
.claude/skills/onenote/
...
rsync -av --exclude='config.json' .claude/skills/onenote/ /path/to/other-project/.claude/skills/onenote/
```
...
- **`-PageName` 过滤**：PS1 先用 `[regex]::Escape()` 转义 coreName（保留 `^$` 锚点），再 `-match` 匹配。**调用者传原始 page name，不要预转义**。自动 forward-scan 包含子页面（pageLevel 更深的后续页面）
