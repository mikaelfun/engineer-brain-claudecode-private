...
function getCliVersion(cliPath: string, env?: Record<string, string>): string | undefined {
  try {
    // Bug 1 fix: On Windows, .js/.mjs files need `node` prefix to execute correctly
...
function checkRequiredFlags(cliPath: string, env?: Record<string, string>): string[] {
  let helpText: string;
  try {
    // Bug 7 fix: Same node prefix for .js/.mjs paths
    const helpCmd = cliPath.match(/\.(m?js)$/i)
      ? `node "${cliPath}" --help`
      : `"${cliPath}" --help`;
    helpText = execSync(helpCmd, {
      encoding: 'utf-8',
      timeout: 10_000,
      env: env || buildSubprocessEnv(),
      stdio: ['pipe', 'pipe', 'pipe'],
...
function isExecutable(p: string): boolean {
  try {
    // Bug 8 fix: Windows NTFS has no execute bit; use R_OK instead of X_OK
        ...
        (async () => {
          ...
          try {
            ...
            const queryOptions: Record<string, unknown> = {
              ...
              // settingSources: ['project'],  // TODO: causes streaming cards to break — investigate
