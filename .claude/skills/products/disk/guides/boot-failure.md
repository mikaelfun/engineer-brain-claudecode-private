# Disk VM Boot Failures — 排查速查

**来源数**: 16 | **21V**: 全部适用
**最后更新**: 2026-04-07
**关键词**: 0xc00000ba, 0xc0000225, 2tb-limit, active-partition, auditd, bcd, bcdboot, bcdedit, boot-error, boot-failure
...
1. Storage account high E2E latency (MAX up to 24s) on GetBlob operations - random  → 1) Check Jarvis Shoebox API Dashboard for E2E vs server latency `[来源: onenote]`
2. Linux VM shuts down unexpectedly and fails to boot when OS disk is full. Serial  → 1) Access OS disk via az vm repair or recovery VM `[来源: mslearn]`
3. Cannot resize MBR partition beyond 2 TB using fdisk on Linux VM data disk. fdisk → Convert partition from MBR to GPT using gdisk: 1) Take snapshot `[来源: mslearn]`
4. Cannot resize (expand) a shared disk while it is attached to multiple VMs. Error → To resize a shared disk: 1) Stop/deallocate ALL VMs attached to the shared disk, OR detach the disk  `[来源: mslearn]`
5. Windows VM fails to boot with error 0xc00000ba: 'The operating system couldn't b → 1) Attach OS disk to recovery VM as data disk `[来源: mslearn]`

> 本 topic 有融合排查指南，含完整排查流程和 Kusto 查询模板
> → [完整排查流程](details/boot-failure.md#排查流程)
