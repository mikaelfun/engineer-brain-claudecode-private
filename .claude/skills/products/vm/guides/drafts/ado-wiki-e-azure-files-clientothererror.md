---
source: ado-wiki
sourceRef: "Supportability/AzureIaaSVM/AzureIaaSVM:/SME Topics/Azure Files All Topics/TSGs/Azure Files ClientOtherError_Storage"
sourceUrl: "https://dev.azure.com/Supportability/AzureIaaSVM/_wiki/wikis/AzureIaaSVM?pagePath=%2FSME%20Topics%2FAzure%20Files%20All%20Topics%2FTSGs%2FAzure%20Files%20ClientOtherError_Storage"
...
# Troubleshooting ClientOtherErrors in Azure Files
...
This article lists the ClientOtherErrors you might encounter when using SMB Azure file shares. In general, ClientOtherErrors are mostly harmless and expected errors. Requests fail, but the system continues to behave as expected. It's normal to see a significant amount of these errors logged.
 ...
 Public Document: [Troubleshoot ClientOtherErrors in Azure Files](https://learn.microsoft.com/en-us/troubleshoot/azure/azure-storage/files/security/files-client-other-errors)
...
### Internally
To troubleshoot ClientOtherErrors internally:
1. [Query Storage Front End Logs](https://supportability.visualstudio.com/AzureIaaSVM/_wiki/wikis/AzureIaaSVM/690089)
    * Leverage `XSMBPerfMetric` table for SMB requests.
    * Check specifically for `Operation`, `Status`, `InternalStatus` and `ActivityId`
2. [Query Storage Verbose Logs](https://supportability.visualstudio.com/AzureIaaSVM/_wiki/wikis/AzureIaaSVM/690091) by `ActivityId`


### Externally (Customers)
To troubleshoot ClientOtherErrors, you can create a diagnostic setting and use [Azure Monitor](https://learn.microsoft.com/en-us/azure/storage/files/storage-files-monitoring) for reporting. You can also [analyze logs](https://learn.microsoft.com/en-us/azure/storage/files/analyze-files-metrics?tabs=azure-portal#analyze-logs) to view failed requests, including ClientOtherErrors, or [use Kusto queries](https://learn.microsoft.com/en-us/azure/storage/files/analyze-files-metrics?tabs=azure-portal#sample-kusto-queries).

You can also collect a [ProcMon trace](https://learn.microsoft.com/en-us/sysinternals/downloads/procmon) from a client that matches the IP address shown in the logs. Add a filter to see only traffic to Azure Files.
...
| ChangeNotify | STATUS_QUOTA_EXCEEDED | Follow [STATUS_QUOTA_EXCEEDED](https://supportability.visualstudio.com/AzureIaaSVM/_wiki/wikis/AzureIaaSVM/495825) TSG  |
