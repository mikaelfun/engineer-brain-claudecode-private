# Networking AppGW HTTP 4xx 错误 — 排查速查
...
## 快速排查路径
1. Customer must reduce URL length to stay within the 32KB limit. See Azure subscription service limits `[来源: ado-wiki]`
2. Associate a routing rule with the listener before testing frontend HTTPS connectivity. A listener wi `[来源: ado-wiki]`
3. Collect HTTPERR logs from %windir%/System32/logfiles/HTTPERR/HTTPERR#.LOG on AppGW V1 instances via  `[来源: ado-wiki]`
4. Check request filtering settings limits. Current defaults: maxAllowedContentLength=2147483647, maxUr `[来源: ado-wiki]`
5. Fix the client application to send RFC-compliant HTTP requests. Review HAR traces from client side t `[来源: ado-wiki]`
