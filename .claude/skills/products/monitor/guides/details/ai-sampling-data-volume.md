# Monitor Application Insights 采样与数据量控制 - Comprehensive Troubleshooting Guide

**Entries**: 15 | **Drafts fused**: 8 | **Kusto queries**: 0
**Draft sources**: ado-wiki-a-identify-if-sampling-is-enabled.md, ado-wiki-a-manage-sampling-in-azure-functions.md, ado-wiki-a-manage-sampling-net-framework.md, ado-wiki-a-manage-sampling-with-net-core.md, ado-wiki-a-sampling-exploration-dotnet-core.md, ado-wiki-b-build-simple-load-test-validate-sampling.md, ado-wiki-c-manage-sampling-with-nodejs.md, ado-wiki-e-sampling.md
...
### Step 1: All telemetry data stops flowing into Application Insights tables periodically and resumes on a daily cycle - data ingestion follows a pattern of stopping and restarting each day

**Solution**: 1) Check Throttle Status tab in ASC to confirm Daily Cap is being hit. 2) Use Read Aggregate by Dropped By Reason to identify 'Quota Exceed / Q' drops. 3) Check both AI and Log Analytics daily caps (whichever is lower takes effect first). 4) Remediate by: increasing Daily Cap, implementing more a...

`[Source: ADO Wiki, Score: 8.5]`

### Step 2: All telemetry stops flowing into Application Insights tables periodically then resumes daily after the Daily Cap reset time; pattern repeats every day

**Solution**: Check throttle status in ASC (Use Throttle Status tab); check Read Aggregate by Dropped By Reason for quota exceeded (Q) indicator; verify both AI and LA daily caps; increase daily cap, implement sampling, or filter noisy telemetry at SDK source to stay under limit

`[Source: ADO Wiki, Score: 8.5]`

### Step 3: RPS drop or CPU increase immediately after application startup under high RPS with Application Insights SDK

**Solution**: Configure AdaptiveSamplingTelemetryProcessor with InitialSamplingPercentage set to desired initial rate, or shorten evaluation period / SamplingPercentageDecreaseTimeout.

`[Source: ADO Wiki, Score: 8.5]`

### Step 4: Continuous CPU/RPS degradation throughout application lifetime at high RPS with Application Insights SDK

**Solution**: 1) 1-10% overhead is expected range. 2) Ensure adaptive sampling is enabled and rates are correct (check ItemCount in Request table). 3) Scale up/out to keep CPU below 90% without AI so with AI it stays under 100%. 4) Remove unnecessary Telemetry Initializers.

`[Source: ADO Wiki, Score: 8.5]`

### Step 5: Ingestion sampling is configured in Azure portal but does not take effect; telemetry volume remains unchanged

**Solution**: Check if SDK-level sampling is configured in application code or config. If SDK sampling is enabled, ingestion sampling will be ignored. Remove SDK sampling first if ingestion sampling is desired.
