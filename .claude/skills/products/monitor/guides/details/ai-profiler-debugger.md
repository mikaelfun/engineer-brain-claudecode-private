# Monitor Application Insights Profiler 与 Snapshot Debugger - Comprehensive Troubleshooting Guide

**Entries**: 18 | **Drafts fused**: 9 | **Kusto queries**: 0
**Draft sources**: ado-wiki-a-determine-if-snapshot-debugger-is-enabled.md, ado-wiki-a-Profiler-Configuration-Performance-Impact.md, ado-wiki-a-Profiler-Traces-Not-Collected.md, ado-wiki-a-snapshot-debugger-log-analysis.md, ado-wiki-a-Snapshot-Debugger-Snapshots-Not-Collected.md, ado-wiki-b-profiler-analyzing-traces.md, ado-wiki-b-profiler-support-boundaries.md, ado-wiki-d-understanding-profiler-data.md, ado-wiki-g-collect-profiler-traces-app-services.md
...
### Step 1: No SQL Dependency telemetry in Application Insights for .NET Framework 4.x application using Microsoft.Data.SqlClient with Profiler enabled. Other telemetry types (Requests, Traces, some Dependenci...

**Solution**: Option 1: Disable the Profiler — SDK falls back to EventSource which supports Microsoft.Data.SqlClient. For App Service: stop ApplicationInsightsProfiler3 webjob. For Azure VMs: disable InstrumentationEngine via Application Insights Agent. Option 2: Switch application code from Microsoft.Data.Sql...

`[Source: ADO Wiki, Score: 8.5]`

### Step 2: Application Insights Profiler not collecting traces on Azure VM/VMSS after enabling App Insights VM Extension. Bootstrap logs show error: Standard Collector uninstallation failed with exit code 160...

**Solution**: Uninstall Visual Studio or Visual Studio Remote Debugging Tools from the VM. Use a clean VM without Visual Studio installed. Profiler will then install successfully and attach to .NET Framework or .NET Core apps in IIS. The Profiler team is working on a fix for a future VM extension version.

`[Source: ADO Wiki, Score: 8.5]`

### Step 3: Application Insights Profiler emits '401 Unauthorized' exception in telemetry between StartProfiler triggered and StopProfiler triggered trace events; profiler fails to upload traces

**Solution**: Configure Bring Your Own Storage (BYOS) for Application Insights Profiler. BYOS routes profiler uploads through a customer-controlled storage account accessible within the private link scope. Reference: https://learn.microsoft.com/en-us/azure/azure-monitor/profiler/profiler-bring-your-own-storage...

`[Source: ADO Wiki, Score: 8.5]`

### Step 4: 'Profile Traces' button in Azure Portal is disabled/grayed out even though Application Insights Profiler appears to be running and emitting telemetry (StartProfiler/StopProfiler events visible)

**Solution**: Run KQL in ASC Customer Query Data tab to verify: let RQID=(){union traces, customEvents | where timestamp > ago(3d) | where sdkVersion startswith 'w_ap' or name in ('ServiceProfilerSample','ServiceProfilerIndex') | extend RequestId = tostring(customDimensions.RequestId) | where isempty(RequestId...

`[Source: ADO Wiki, Score: 8.5]`

### Step 5: Application Insights Profiler telemetry shows trace message 'Upload trace was canceled as no activity detected from target iKey'; profiler sessions run but no trace is uploaded or available in portal

**Solution**: Ensure the application is receiving HTTP requests during the Profiler sampling window. Use the Remote-Immediate trigger (manual on-demand profiling) from the Azure Portal during a period of known active traffic. Review trigger settings (Memory, CPU, Default-Sampling thresholds) via: https://learn...
