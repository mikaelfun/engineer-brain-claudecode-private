...
Agent-A 生成速查表时，**每条必须独立计算** 4 个维度的分数，然后求和。
...
| source | 分数 | 理由 |
|--------|------|------|
| `onenote` | 3.0 | 一线排查经验，经过团队验证 |
| `ado-wiki` | 2.5 | 产品组标准 TSG |
| `case` (rootCause 完整) | 2.0 | 实际案例验证 |
| `contentidea-kb` | 2.0 | 已发布 KB 文章 |
| `mslearn` | 1.5 | 官方通用文档 |
| `case` (rootCause 模糊) | 1.0 | 案例但原因不确定 |

#### 维度 2: Recency（0-2）

按条目的 `date` 字段计算与当前日期的时间差：
...
⚠️ **禁止忽略 date 字段使用默认值**。date 字段为空时此维度记 0。

#### 维度 3: Validation Strength（0-3）

按该条目在 **同 topic 内** 的交叉验证情况：

| 验证强度 | 分数 | 判定条件 |
|----------|------|---------|
| 多源交叉验证 | 3.0 | 同 topic 内 ≥2 个不同 source 的条目描述相同 rootCause |
| 单源 + 强证据 | 2.0 | 条目有 KQL 查询、截图引用、或明确复现步骤 |
| 单源文档 | 1.0 | 来自文档但无独立验证 |
| 无法验证 | 0 | rootCause 为 null 或纯路由条目 |

判定逻辑：
- "相同 rootCause" = 同 topic 内另一条目的 rootCause 与本条 rootCause 语义相似度 > 60%
- "强证据" = solution 中包含 KQL 代码块 或 solution 有分步骤操作指引（包含 ≥3 个操作步骤）

#### 维度 4: 21V Applicability（0-2）

按条目的 `21vApplicable` 字段：

| 标注 | 分数 | 含义 |
|------|------|------|
| `true` + 明确标注 Mooncake | 2.0 | 明确经过 21V 验证 |
| `true`（默认标注） | 1.0 | 通用/未专门验证 |
| `null` | 0.5 | 待分类 |
| `false` | 0 | 不适用 Mooncake |

区分"明确标注"与"默认标注"：如果条目 tags 包含 `21v-verified` 或 `mooncake` 或 source 为 `21v-gap` → 明确标注 2.0；否则 → 默认标注 1.0。

### 总分计算

```
总分 = Source Quality + Recency + Validation Strength + 21V Applicability
范围：0 ~ 10
```
...
|----------|-------|------|
| 8.0 - 10.0 | 🟢 | 可直接采信 |
| 5.0 - 7.9 | 🔵 | 可参考，建议验证关键步骤 |
| 3.0 - 4.9 | 🟡 | 仅供方向参考 |
| 0 - 2.9 | ⚪ | 可能已过时，谨慎使用 |

### 执行约束

- ❌ **禁止**按 source 统一赋分（如 "ADO Wiki 全部 7.0"）
- ❌ **禁止**忽略 date 字段
- ❌ **禁止**跳过 Validation Strength 维度给默认值
- ✅ 必须逐条独立计算 4 个维度
- ✅ 同一 source 的不同条目允许且应该有不同分数
- ✅ 速查表中显示总分 + badge，不显示维度明细（维度明细在 details/ 中展示）

### 示例

| 条目 | Source | Date | 同topic交叉 | 21V | 计算 | 总分 |
|------|--------|------|-------------|-----|------|------|
| vm-onenote-012 | onenote | 2026-02 | 有 wiki 验证 | 明确标注 | 3+2+3+2 | 🟢 10.0 |
| vm-ado-wiki-045 | ado-wiki | 2025-06 | 单源+KQL | 默认true | 2.5+1.5+2+1 | 🔵 7.0 |
| vm-mslearn-003 | mslearn | 2023-01 | 无交叉 | 默认true | 1.5+0.5+1+1 | 🟡 4.0 |
| vm-ado-wiki-089 | ado-wiki | 2026-03 | 有 ON 验证 | 默认true | 2.5+2+3+1 | 🟢 8.5 |
