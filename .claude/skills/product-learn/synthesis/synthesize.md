...
- **自动（全量）**：某产品全部 5 个 source 均 `"idle"` 后自动触发
- **自动（早期）**：某产品 `known-issues.jsonl` 条目数 ≥ 50 且至少 2 个 source 已 `"idle"` → 触发早期合成
...
- `synthesizeState.lastEntryCount > 0`（增量合成）→ **Topic 级按需模式**：

**Topic 级变化检测**（逐 topic 检查，替代旧的全量 20 条阈值）：

```python
topicsToRegen = []
allEntryIds = set(e['id'] for e in all_jsonl_entries)
previousIds = set(synthesizeState.get('synthesizedEntryIds', []))
newEntryIds = allEntryIds - previousIds

# 读取当前 draft 和 kql 文件的 content hash
currentDraftHashes = {path: hash(read(path)) for path in glob('guides/drafts/*.md')}
currentKqlHashes = {path: hash(read(path)) for path in glob(f'.claude/skills/kusto/{product}/references/queries/*.md')}
previousDraftHashes = synthesizeState.get('draftHashes', {})
previousKqlHashes = synthesizeState.get('kqlHashes', {})

for topic in topic_plan['topics']:
    reasons = []
    
    # 触发条件 1：该 topic 有新 JSONL 条目（哪怕只有 1 条）
    topicNewEntries = [eid for eid in topic['entryIds'] if eid in newEntryIds]
    if topicNewEntries:
        reasons.append(f"+{len(topicNewEntries)} entries")
    
    # 触发条件 2：该 topic 关联的 draft 文件内容 hash 变了
    for draftPath in topic.get('draftPaths', []):
        if currentDraftHashes.get(draftPath) != previousDraftHashes.get(draftPath):
            reasons.append(f"draft changed: {draftPath}")
    
    # 触发条件 3：该 topic 关联的 Kusto query 文件内容 hash 变了
    for kqlFile in topic.get('kustoQueryFiles', []):
        if currentKqlHashes.get(kqlFile) != previousKqlHashes.get(kqlFile):
            reasons.append(f"kql changed: {kqlFile}")
    
    if reasons:
        topicsToRegen.append({'topic': topic, 'reasons': reasons})
```

**三个触发条件**（任一命中即触发该 topic 重生成）：

| # | 触发条件 | 检测方式 | 影响范围 |
|---|---------|---------|---------|
| 1 | 该 topic 有新 JSONL 条目 | `entryIds ∩ newEntryIds` 非空 | Agent-A + B + C |
| 2 | 该 topic 的 draft 文件内容变化 | `draftHashes` 对比 | Agent-C（workflow 重生成）|
| 3 | 该 topic 的 Kusto query 文件变化 | `kqlHashes` 对比 | Agent-C（workflow 重生成）|

**Unassigned 条目处理**（新条目未匹配任何已有 topic）：

```python
# 新条目匹配已有 topic 的 keywords
assignedNewIds = set()
for topic in topic_plan['topics']:
    for eid in newEntryIds:
        entry = entries_by_id[eid]
        if any(kw in entry['symptom'].lower() or kw in entry.get('tags', []) 
               for kw in topic.get('keywords', [])):
            if eid not in set(topic['entryIds']):
                topic['entryIds'].append(eid)
                assignedNewIds.add(eid)

unassignedPool = newEntryIds - assignedNewIds

if len(unassignedPool) >= 10:
    # 触发小范围聚类（仅对 unassigned 条目，不重聚类全量）
    # spawn 1 个聚类 agent，输入仅 unassigned 条目
    # 产出新 topic 或归入已有 topic
    log(f'SYNTHESIZE: {len(unassignedPool)} unassigned entries → small-range clustering')
elif len(unassignedPool) > 0:
    log(f'SYNTHESIZE: {len(unassignedPool)} unassigned entries (< 10, deferred)')
```

**增量合成逻辑**：
1. 对 `topicsToRegen` 中的 topic → **只重新生成这些 topic** 的速查表、详情和工作流
2. 未变化的 topic → **完全跳过**（不 spawn 任何 agent）
3. 新条目匹配到已有 topic → 追加到 `entryIds`，标记该 topic 需重生成
4. 新条目无法匹配（unassigned 池 ≥ 10）→ 小范围聚类产出新 topic
5. 更新 `synthesizeState`：
   ...
   - `draftHashes` = 所有 draft 文件的内容 hash
   - `kqlHashes` = 所有 Kusto query 文件的内容 hash
   - `topicRegenLog` += 本次重生成的 topic 记录
...
**增量模式下，仅对 `topicsToRegen` 中的 topic spawn agent**。未变化的 topic 完全跳过：

```
增量模式日志：
  SYNTHESIZE: Skipping {N} unchanged topics, regenerating {M} topics:
    - {topic1}: +2 entries, draft changed
    - {topic2}: kql changed: service-healing.md
```

对每个需重生成的 topic，spawn 最多三个 agent 并行工作：
...
- `synthesizeState` 更新：
  ```json
  {
    ...
    "kqlHashes": {"service-healing.md": "abc789", "vm-operations.md": "xyz012", ...},
    "topicRegenLog": [
      {"topic": "vm-start-stop", "reasons": ["+2 entries", "draft changed: onenote-service-healing-tsg.md"], "at": "2026-04-19"},
      {"topic": "vm-gpu", "reasons": ["+1 entries"], "at": "2026-04-19"}
    ],
  ...
  | 字段 | 类型 | 说明 |
  |------|------|------|
  | `kqlHashes` | `dict<filename, hash>` | Kusto query 文件的内容 hash，增量合成时对比检测变化 |
  | `topicRegenLog` | `array<{topic, reasons, at}>` | 每次增量合成重生成的 topic 审计日志 |
