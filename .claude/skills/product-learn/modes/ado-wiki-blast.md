...
**文件**: `.claude/skills/products/{product}/.enrich/claimed-pages.json`
...
1. 读取 `.claude/skills/products/{product}/.enrich/scanned-ado-wiki.json`
...
- 扫描记录: .claude/skills/products/{product}/.enrich/blast-temp/scanned-ado-wiki-{batchId}.json
- ID 格式: {product}-ado-wiki-{batchId}-r{round}-{seq:03d}（round 从已有 JSONL 最大 round+1 开始，无已有则 r1）
- 草稿前缀: .claude/skills/products/{product}/guides/drafts/ado-wiki-{batchId}-{title}.md
...
- **禁止在 `.claude/skills/products/` 根目录写任何临时文件**
...
3. **构建 key + pathScope 过滤**
   ...
   **pathScope 过滤**（关键步骤！）：
   从 `.claude/skills/products/{product}/.enrich/wiki-scope.json` 读取对应 wiki 的 scope 规则（不再从 config.json 读取）：
   ```python
   # 读取 scope 配置
