...
- **时间戳** `date +%s > "{caseDir}/.casework/logs/.t_{stepName}"`，**禁止**为日志或时间戳单独发 Bash
- **日志目录约定（ISS-220）**：所有日志统一在 `{caseDir}/.casework/logs/` 下。包括：
  - `casework.log` — STEP 级别流程日志（LLM agent 写）
  - `.t_*` — timing markers（LLM agent 写）
  - `d365.log` / `teams.log` / `onenote.log` / `attachments.log` — data-refresh 组件日志（bash 脚本写）
  - 不再使用外层 `{caseDir}/logs/`
...
rm -f "$CASE_DIR/.casework/logs/.t_"*
date +%s > "$CASE_DIR/.casework/logs/.t_start"
date +%s > "$CASE_DIR/.casework/logs/.t_changegate_start"
...
date +%s > "$CASE_DIR/.casework/logs/.t_changegate_end"
```
