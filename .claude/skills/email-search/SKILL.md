...
**结果去重**：
Graph API `$search` 会返回同一封邮件的多个副本（Inbox 收件 + Sent Items 发件各一份）。必须去重：
- 用 `subject + sentDateTime` 作为去重 key
- 相同 key 的邮件只保留第一个（id 不同但内容相同）
- 记录去重前后数量：`Found 37 emails, deduplicated to 19`
...
**注意事项**：
- 单封邮件失败（重试后仍失败）不阻塞其他邮件获取 — 记录失败的 messageId 和错误原因
