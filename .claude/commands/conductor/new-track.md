...
2. **Classify each criterion** using the rules from `conductor/workflow.md` Step 2a:
   ...
   - **API** — Backend behavior: new endpoint, changed response shape, data persistence
   - **Skip** — Backend-only with no UI surface, or already fully covered by unit tests
3. **Write high-level test steps** — describe WHAT to test, not the exact Playwright code (the verify agent will generate code later)
4. **Reference the safety red lines** from CLAUDE.md — never plan tests that click Full Process, Troubleshoot, Draft Email, or Execute buttons
5. **For Interaction tests**, prefer API-seeded data patterns (create test data via API, then interact via UI)
...
| 4 | Internal refactor — no UI change | Skip | Backend-only, covered by unit tests |
