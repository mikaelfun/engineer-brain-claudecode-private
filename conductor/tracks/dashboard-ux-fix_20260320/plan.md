...
|---|---------------------|-----------|------------|
| 1 | 每个 case 的每种操作类型最多一个活跃 session，UI 即时禁用已触发按钮 | Visual | Navigate to page → screenshot → verify visual matches spec |
| 2 | 操作触发后有实时步骤级进度显示（SSE 驱动） | Visual | Navigate to page → screenshot → verify visual matches spec |
| 3 | 页面刷新后能从后端恢复活跃 session 状态和历史消息 | Visual | Navigate to page → screenshot → verify visual matches spec |
| 4 | Todo 页面可正常加载 per-case todo 数据，移除 legacy 代码 | Visual | Navigate to page → screenshot → verify visual matches spec |
| 5 | Workspace Not Found 仅在真正无 workspace 时显示 | Visual | Navigate to page → screenshot → verify visual matches spec |
| 6 | Analysis tab 能正确检测并显示 analysis/ 目录下所有内容 | Visual | Navigate to page → screenshot → verify visual matches spec |
| 7 | Case 标题完整显示或有 tooltip | Visual | Navigate to page → screenshot → verify visual matches spec |
| 8 | 所有 Critical 和 High 问题修复后通过手动 smoke test | Visual | Navigate to page → screenshot → verify visual matches spec |
