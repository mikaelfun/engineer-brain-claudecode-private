# Networking AppGW 重定向与重写规则 — 排查速查
...
## 快速排查路径
1. Use Application Gateway rewrite rules instead of redirect rules to an external site. Rewrite rules p `[来源: ado-wiki]`
2. Three options: (1) Add a custom domain to the backend app service matching the AppGW frontend domain `[来源: ado-wiki]`
3. Use Azure Resource Explorer to modify the redirect configuration JSON and explicitly set `"includePa `[来源: ado-wiki]`
4. This is expected by-design behavior. Customer should: 1) Use trailing slash in their URLs; 2) Update `[来源: ado-wiki]`
5. Review all redirect configurations and consolidate duplicates — multiple path-based rules pointing t `[来源: ado-wiki]`
