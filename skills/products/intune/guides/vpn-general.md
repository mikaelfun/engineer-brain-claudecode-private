...
## 快速排查路径
1. 1) Connect to MSFT-AzVPN-Manual (or usual work VPN). 2) Open the ModernRDP app. 3) Click on the name of the workspace needing JIT. 4) Click the 'JIT N `[来源: ADO Wiki]`
2. Disconnect/reconnect VPN while Edge running. For Always-on VPN use airplane mode toggle. Alt: Postpone Android 13 upgrade. `[来源: MS Learn]`
3. To create a custom VPN profile:  Choose your SSO &gt; Put in the server address  Point the bundle Identifier (also known as the VPN identifier)  &gt;& `[来源: ContentIdea KB]`
4. Migrate their iOS devices to Intune on Azure as soon as possible so they can set up VPN profiles for the new VPN client. If they are unable to migrate `[来源: ContentIdea KB]`
