...
## 快速排查路径
1. 1. 重启 storedownloadd 进程或重启设备；2. 修复问题 pkg（确保 PackageInfo 符合要求）；3. macOS 10.14 Mojave 及以后版本已修复此问题 `[来源: ADO Wiki]`
2. 在 Intune 应用配置中启用忽略应用版本（Ignore app version）选项 `[来源: ADO Wiki]`
3. 检查 PackageInfo 确认每个 package 都是 application 类型；如果 pkg 混合了 library 和 app，可能需要分拆或确保主 app bundle 信息正确 `[来源: ADO Wiki]`
4. Review the output of Detection.xml to extract the detection parameters and version for the created .Intunemac file. On a MacOS system with the .intune `[来源: ContentIdea KB]`
5. After   Jamf released 10.6.2, you had to do the following steps to get the client to   re-register: 1. Delete the Jamf Client from the MAC 2. Uninstal `[来源: ContentIdea KB]`
