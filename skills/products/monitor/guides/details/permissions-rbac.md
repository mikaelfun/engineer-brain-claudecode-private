# Monitor 监控权限与 RBAC - Comprehensive Troubleshooting Guide

**Entries**: 20 | **Drafts fused**: 9 | **Kusto queries**: 0
**Draft sources**: ado-wiki-a-ASC-Get-Role-Permissions.md, ado-wiki-a-ASC-Get-User-Role-Permissions-Scope.md, ado-wiki-b-AAD-Permissions-Testing.md, ado-wiki-b-Access-and-permissions-RBAC.md, ado-wiki-b-Check-RBAC-details-from-ASC.md, ado-wiki-b-Check-RBAC-permissions-for-LA-operations.md, ado-wiki-b-Granular-RBAC-TSG.md, ado-wiki-b-how-to-get-user-role-permissions-and-role-assignment-scope.md, ado-wiki-c-Log-Analytics-Search-API-Permissions.md
...
### Step 1: Log search alert rule querying across tenants fails or stops working - cross-tenant target resource no longer supported

**Solution**: Use Azure Lighthouse to assign RBAC roles with adequate access across tenants for the alert rule identity (default or managed). Direct cross-tenant target resource is deprecated.

`[Source: ADO Wiki, Score: 8.5]`

### Step 2: Log search alert rule using linked storage account fails after July 2024 or when using system-assigned managed identity with linked storage

**Solution**: 1) Configure user-assigned managed identity on the alert rule 2) Assign Storage Blob Data Contributor role on the linked storage account (write operations needed for storing log queries) 3) Assign Reader role on the workspace

`[Source: ADO Wiki, Score: 8.5]`

### Step 3: Log search alert rule cross-service query to ADX or ARG fails - default identity does not work for cross-service queries

**Solution**: Switch to system-assigned or user-assigned managed identity. For ADX: Reader on workspace + Reader on ADX resource + database viewer on all queried databases. For ARG: Reader on workspace + Reader on queried resources.

`[Source: ADO Wiki, Score: 8.5]`

### Step 4: Error creating/updating ITSM connection: 'access_denied', 'server_error', or Unauthorized response when authenticating with ServiceNow

**Solution**: Validate authentication by sending a direct token request to ServiceNow using Postman or similar tool with the same credentials. If response is Unauthorized, it's a ServiceNow configuration issue — fix user roles and permissions per the ITSM Connector documentation.

`[Source: ADO Wiki, Score: 8.5]`

### Step 5: Error creating/updating ITSM connection: access_denied, server_error, or Unauthorized response when authenticating with ServiceNow

**Solution**: Validate authentication by sending a direct token request to ServiceNow using Postman or similar tool with the same credentials. If response is Unauthorized, it is a ServiceNow configuration issue - fix user roles and permissions per the ITSM Connector documentation.
