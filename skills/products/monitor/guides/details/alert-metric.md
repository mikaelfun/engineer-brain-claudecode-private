# Monitor 指标告警规则 - Comprehensive Troubleshooting Guide

**Entries**: 18 | **Drafts fused**: 0 | **Kusto queries**: 1
**Draft sources**: 
**Kusto references**: metric-alerts.md
...
### Step 1: VM metric alert triggered but portal metric chart shows value did not exceed the configured threshold. Customer believes alert is false positive.

**Solution**: 1) Set metric chart time granularity to 1 minute when comparing with alert rules to see actual peak values. 2) Check for missing metric data points in the evaluation window. 3) Use Portal's fired alert details to see the exact calculated metric value and timestamp. 4) Jarvis dashboard can query V...

`[Source: OneNote, Score: 9.0]`

### Step 2: Metric alert fires falsely or misses firing; metric value differs between red and black instances of Geneva Metrics stamp

**Solution**: In ASC evaluation logs, add the second instance (-red or -black suffix) to the Datacenter filter, then filter by LogTag in (ThresholdViolated,CombinationEvaluation). Compare ThresholdViolated and metric values between instances. If one shows ThresholdViolated and the other does not → confirmed re...

`[Source: ADO Wiki, Score: 8.5]`

### Step 3: Metric alert fired when it should not have (false alert); metric value at evaluation time differs from resource metric chart value

**Solution**: Check MetricDelay Jarvis dashboard (https://jarvis-west.dc.ad.msft.net/dashboard/MetricTeamInternalMetrics/Sla/MetricDelay). Set Metrics Backend and Environment, filter by evaluation time. For platform metrics check Metric Delay chart; for custom metrics (CUSTOM account or azmon stamp) check Metr...

`[Source: ADO Wiki, Score: 8.5]`

### Step 4: Metric alert remains in fired state and does not resolve automatically when the metric no longer meets the threshold

**Solution**: Metric alert resolves when condition not met for 3 consecutive evaluations (e.g. 15 min for 5-min aggregation granularity). 1) Verify correct metric time series — if rule monitors multiple resources or dimensions, each combination fires/resolves independently. 2) Ensure autoMitigate=true / "Autom...

`[Source: ADO Wiki, Score: 8.5]`

### Step 5: Availability test alert fires unexpectedly (false positive); customer believes the metric conditions did not warrant an alert to fire

**Solution**: 1) In Azure Portal, open the fired alert and review "Why did this alert fire?" section to see Metric Value vs Threshold at fire time. 2) Ensure Aggregation in metric chart matches Aggregation type in alert rule. 3) Ensure Time granularity matches Lookback period (not set to automatic). 4) Verify ...
