# Monitor AMA Linux 代理排查 - Comprehensive Troubleshooting Guide

**Entries**: 19 | **Drafts fused**: 50 | **Kusto queries**: 0
**Draft sources**: ado-wiki-a-AMA-Linux-Arc-Extension-Download.md, ado-wiki-a-AMA-Linux-Check-Cache-Size.md, ado-wiki-a-AMA-Linux-Check-Distro-Kernel.md, ado-wiki-a-AMA-Linux-Check-Proxy-Settings.md, ado-wiki-a-AMA-Linux-TSG-Collection-Text-Logs.md, ado-wiki-a-AMA-Linux-TSG-Configuration-Azure-Arc.md, ado-wiki-a-AMA-Linux-TSG-Configuration-Azure-VM.md, ado-wiki-a-AMA-Linux-TSG-Installation-Azure-Arc.md, ado-wiki-a-AMA-Linux-TSG-Installation-Azure-VM.md, ado-wiki-a-AMA-Linux-VM-GuestAgent-Extension-Download.md
...
### Step 1: Need to collect Azure Monitor Agent (AMA) logs on Linux VM for troubleshooting data collection issues.

**Solution**: Collect AMA logs from: extension /var/log/azure/Microsoft.Azure.Monitor.AzureMonitorLinuxAgent/; waagent.log; AMA extension /var/lib/waagent/Microsoft.Azure.Monitor.AzureMonitorLinuxAgent-ver/; mdsd logs /var/opt/microsoft/azuremonitoragent/log/; events /var/opt/microsoft/azuremonitoragent/events...

`[Source: OneNote, Score: 9.0]`

### Step 2: AMA fails to parse syslog messages. mdsd.info shows 'Invalid field APPNAME' errors. Syslog not ingested to LAW.

**Solution**: 1) Update the source application to generate RFC 3164 compliant syslog messages (APPNAME should be alphanumeric only). 2) Verify DCR includes the correct syslog facility - the messages may be logged under a different facility than expected (e.g., user.notice instead of daemon.info). Update DCR to...

`[Source: ADO Wiki, Score: 8.5]`

### Step 3: AMA not collecting syslog on Linux. Syslog daemon config file for AMA forwarding is missing (/etc/rsyslog.d/10-AzureMonitorAgent-omfwd.conf or /etc/syslog-ng/conf.d/azuremonitoragent-tcp.conf).

**Solution**: Verify: 1) DCR has a valid syslog data source configured and is associated to the VM resource. 2) rsyslog or syslog-ng package is installed on the VM. Once both conditions are met, restart AMA ('systemctl restart azuremonitoragent') and verify the config file is created.

`[Source: ADO Wiki, Score: 8.5]`

### Step 4: AMA syslog parsing fails - Invalid APPNAME in mdsd.info

**Solution**: Update app for RFC 3164 compliance; update DCR facility

`[Source: ADO Wiki, Score: 8.5]`

### Step 5: AMA not collecting syslog - forwarding config missing

**Solution**: Verify DCR + daemon package; restart AMA
