---
source: ado-wiki
sourceRef: "Supportability/AzureMonitor/AzureMonitor.wiki:/Monitor Agents/Azure Monitor Agent (AMA) - NEW STRUCTURE/How-To/AMA: HT: Review network trace"
sourceUrl: "https://dev.azure.com/Supportability/AzureMonitor/_wiki/wikis/AzureMonitor.wiki?pagePath=%2FMonitor%20Agents%2FAzure%20Monitor%20Agent%20(AMA)%20-%20NEW%20STRUCTURE%2FHow-To%2FAMA%3A%20HT%3A%20Review%20network%20trace"
...
# Overview
This **How To** article demonstrates how to review a network trace for common scenarios. 

#Wireshark Display details
[AMA: HT: Wireshark display - Overview](https://supportability.visualstudio.com/AzureMonitor/_wiki/wikis/AzureMonitor.wiki/1785327/AMA-HT-Wireshark-display)

# Scenario: Syslog daemon (incoming)
If a customer uses non-standard ports for syslog or uses TLS encryption, those conversations will deviate from what is demonstrated below. This article assumes Syslog is using port 514 on UDP and/or TCP.

The below examples were generated using the following logger commands:
```
logger --udp --rfc5424 -n servername --port 514 -p local0.notice Message from $(hostname) $(date)
logger --udp --rfc3164 -n servername --port 514 -p local0.notice Message from $(hostname) $(date)
logger --tcp --rfc5424 -n servername --port 514 -p local0.notice Message from $(hostname) $(date)
logger --tcp --rfc3164 -n servername --port 514 -p local0.notice Message from $(hostname) $(date)
```
The network trace was captured using this command:
```
tcpdump -s 0 -i any -w /tmp/AMASyslog$(date +'%Y-%m-%dT%H').pcap
```
##Syslog via UDP

Determine the port used by the Syslog daemon by following [AMA Linux: HT: Check the listening ports of a process - Overview](https://supportability.visualstudio.com/AzureMonitor/_wiki/wikis/AzureMonitor.wiki/1779079/AMA-Linux-HT-Check-the-listening-ports-of-a-process)
...
Example of UDP Syslog message on port 514 using RFC5424
Generated using the command:
```
logger --udp --rfc5424 -n servername -p local0.notice Message from $(hostname) $(date)
```
![image.png](/.attachments/image-22aa352e-118b-427c-8f98-38f16f0e68e6.png)
**Source:** Sender of the Syslog message.  This is the originator of the message
**Destination:** Receiver of the Syslog message.  This is the Syslog collector, likely the machine where this was captured.
**Info:** The Info field displays a friendly version of the Syslog message.  Note the parsed version of the Facility and Severity fields
...
logger --tcp --rfc5424 -n servername --port 514 -p local0.notice Message from $(hostname) $(date)
```
![image.png](/.attachments/image-2f347b8c-63d5-4c25-bb4c-aec803fd1f3c.png)

The above screenshot is that of a specific TCP stream of Syslog data. The stream starts with a 3-way handshake to negotiate the parameters of the TCP session. The 3-way handshake used to open the TCP session may not always be in the network trace. It's possible and that the handshake occurred before the trace was started or the data was windowed out of the buffer. This is displayed as informational only.  For our purposes, it is not important. 

**Source:** Sender of the TCP packet.  
**Destination:** Receiver of the TCP packet.
**Info:** The Info field displays a friendly version of the packet.  If the packet contains a Syslog message, the details of that message will be present.  If the packet is only a TCP ACK, details about the TCP packet will be displayed.



# Scenario: Syslog daemon (outgoing)

Filter:
```
tcp.port==28330 or syslog
```
![image.png](/.attachments/image-a6fe9c8b-eb86-4335-9421-f3967c0265d5.png)

The selected packet in the above screenshot is an example of a TCP packet sent from the Syslog daemon, forwarded to the Azure Monitor Agent.  TCP port 28330 is used when sending data from Syslog to Azure Monitor Agent. Note how the packet is not parsed like a normal Syslog message.  This is because it isn't using the well-known Syslog port.  Wireshark is not configured to parse TCP port 28330 as Syslog. Compare this packet to the packet just above it.  The packet just above it is on port 514, the default for Syslog. That packet is parsed as Syslog since Wireshark understands that port 514 is a well-known port for Syslog.  

## Known Issues
[AMA Linux: KI: Rsyslog "messages lost due to rate-limiting" - Overview](https://supportability.visualstudio.com/AzureMonitor/_wiki/wikis/AzureMonitor.wiki/1924199/AMA-Linux-KI-Rsyslog-messages-lost-due-to-rate-limiting-)

# Scenario: ODS Connection
