...
| 字段名 | 类型 | 说明 | 示例 |
|--------|------|------|------|
| DataCollectedOn | datetime | 数据采集时间 | 2026-01-01T00:00:00Z |
| NodeId | string | 节点 ID | guid |
| ClusterId | string | 集群 ID | 集群名 |
...
| OSDiskNumber | int | OS 磁盘编号 | 0 |
| Size | long | 磁盘大小（字节） | - |
| IsSsd | bool | 是否为 SSD | true, false |
| MediaType | string | 媒体类型 | - |
| InterfaceType | string | 接口类型 | - |
| PredictFailure | bool | 是否预测故障 | true, false |
| SMARTSupported | bool | 是否支持 SMART | true, false |
...
| where NodeId == "{nodeId}"
| project DataCollectedOn, NodeId, SCSIPort, SCSIBus, SCSIAddress, SCSILUN, DriveSerialNumber, 
         DriveProductId, FirmwareRevision, DriveBusType, SystemDrive, OSDiskNumber, 
         Size, IsSsd, MediaType, PredictFailure
```
...
| join kind = inner (cluster("azuredcmmc.kusto.chinacloudapi.cn").database("AzureDcmDb").dcmInventoryComponentDiskDirect 
...
| project PreciseTimeStamp, EventId, Description, Cluster, NodeId, 
  ...
  OSDiskNumber, Size, IsSsd, ProviderName, Channel
);
