...
mkdir -p "$CASE_DIR/logs" "$CASE_DIR/teams" "$CASE_DIR/onenote" "$CASE_DIR/context"
rm -f "$CASE_DIR/execution-plan.json"
...
# Write execution-plan.json using relative path
plan_path = os.path.join('$CASE_DIR', 'execution-plan.json')
...
if [ "$ATTACH_COUNT" -gt 0 ]; then
  pwsh -NoProfile -File "$CD/skills/d365-case-ops/scripts/download-attachments.ps1" \
