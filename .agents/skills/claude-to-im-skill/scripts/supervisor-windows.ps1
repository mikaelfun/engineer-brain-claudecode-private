...
# Bug 2 fix: PowerShell 5.1 Join-Path only accepts 2 arguments
...
function Start-Fallback {
    ...
    # Windows fix: Start-Process cannot redirect stdout and stderr to the same file.
    # Use separate stderr log file.
...
# Bug 4 fix: Load config.env into environment before spawning daemon
...
switch ($Command) {
    'start' {
        ...
        if ($svc) {
            ...
            # Bug 5 fix: Increased from 3s to 8s for Feishu WebSocket handshake
            ...
            # Bug 3 fix: $PID is read-only in PowerShell, renamed to $bridgePid
