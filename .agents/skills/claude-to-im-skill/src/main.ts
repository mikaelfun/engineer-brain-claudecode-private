...
async function main(): Promise<void> {
  // Bug 6 fix: loadConfig() parses config.env into an internal Map but never sets
  // process.env. The SDK and resolveClaudeCliPath() read from process.env.
  // Manually inject config.env values into process.env before anything else.
  try {
