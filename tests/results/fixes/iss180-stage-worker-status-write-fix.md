# Fix Report: iss180-stage-worker-status-write

**Test ID:** iss180-stage-worker-status-write
**Fix Type:** framework_fix
**Description:** Fixed json_field handler grep pattern to not match json_field_enum (schema mismatch: json_field intercepted json_field_enum lines causing false positives via grep character class)
**Modified Files:** tests/executors/e2e-runner.sh
**Fixed At:** 2026-03-30T20:17:21Z
**Recipe Used:** none

## What Was Fixed

Fixed json_field handler grep pattern to not match json_field_enum (schema mismatch: json_field intercepted json_field_enum lines causing false positives via grep character class)
...
@@ -578,33 +653,41 @@ run_standalone_bash_assertions() {
     fi
...
+  _flush_bash_assertion
 ...
 # ============================================================
@@ -614,7 +697,9 @@ execute_file_verification() {
...
@@ -803,8 +888,46 @@ verify_case_outputs() {
       fi
     fi
...
+    if echo "$line" | grep -q 'type: .json_field"'; then
       ...
       local field=""
       local expected=""
@@ -854,6 +977,61 @@ verify_case_outputs() {
         esac
       fi
     fi
+
