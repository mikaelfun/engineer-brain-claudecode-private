# Fix Report: STEP_OUTPUT_source_unbound

**Test ID:** STEP_OUTPUT_source_unbound
**Fix Type:** code_bug
**Description:** e2e-runner.sh _flush_bash_check_assertion unconditionally resolves STEP_OUTPUT_N for all types. Fixed by splitting into two-phase case statement.
**Modified Files:** tests/executors/e2e-runner.sh
**Fixed At:** 2026-03-31T22:13:26Z
**Recipe Used:** none

## What Was Fixed

e2e-runner.sh _flush_bash_check_assertion unconditionally resolves STEP_OUTPUT_N for all types. Fixed by splitting into two-phase case statement.
...
@@ -578,43 +659,297 @@ run_standalone_bash_assertions() {
     fi
...
@@ -803,8 +1138,46 @@ verify_case_outputs() {
       fi
     fi
...
+    if echo "$line" | grep -q 'type: .json_field"'; then
       ...
       local field=""
       local expected=""
@@ -854,6 +1227,61 @@ verify_case_outputs() {
         esac
       fi
     fi
+
...
+    fi
 ...
 # ============================================================
@@ -915,6 +1343,7 @@ case "$WORKFLOW_TYPE" in
                 run_standalone_bash_assertions
                 ;;
...
@@ -925,11 +1354,17 @@ ELAPSED=$(get_elapsed_ms)
 OVERALL_STATUS="pass"
