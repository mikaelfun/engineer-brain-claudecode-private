...
parse_steps() {
  ...
  while IFS= read -r line; do
    ...
    if echo "$line" | grep -q "url:"; then
      ...
      # Replace {live_case_id} — fetch a real case ID from the active cases directory
      if echo "$current_url" | grep -q '{live_case_id}'; then
        if [ -z "${LIVE_CASE_ID:-}" ]; then
          # Try to get first active case ID from cases/active/
          local cases_dir="$PROJECT_ROOT/cases/active"
          if [ -d "$cases_dir" ]; then
            LIVE_CASE_ID=$(ls -1 "$cases_dir" 2>/dev/null | head -1)
          fi
          if [ -z "${LIVE_CASE_ID:-}" ]; then
            log_fail "CONTRACT_MISMATCH: {live_case_id} placeholder requires data_source: live but no active cases found"
            current_url=$(echo "$current_url" | sed "s|{live_case_id}|MISSING_LIVE_CASE|g")
          else
            log_info "Resolved {live_case_id} to: $LIVE_CASE_ID"
          fi
        fi
        current_url=$(echo "$current_url" | sed "s|{live_case_id}|${LIVE_CASE_ID:-}|g")
      fi
    fi
    ...
    if echo "$line" | grep -q "auth:"; then
      local auth_val
      auth_val=$(echo "$line" | sed 's/.*auth: *\(.*\)/\1/' | tr -d ' "'"'"'\r')
